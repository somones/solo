...
class applicant 
{
	...
	function get_profession($profession_id_FK){
		$db		=new Database();
		$query="SELECT * FROM `setup_profession` WHERE profession_id = '".$profession_id_FK."'";
		$db->query($query);
	 	$result = $db->resultset();
	 	return $result;
	}

	function get_source($source_id_FK){
		$db		=new Database();
		$query="SELECT * FROM `applicant_source` WHERE source_id = '".$source_id_FK."'";
		$db->query($query);
	 	$result = $db->resultset();
	 	return $result;
	}

	function get_availability_type($applicant_availibility_FK){
		$db		=new Database();
		$query="SELECT * FROM `setup_availability_type` WHERE availability_type_id = '".$applicant_availibility_FK."'";
		$db->query($query);
	 	$result = $db->resultset();
	 	return $result;
	}
	...
	function insert_new_applicant($array,$user_id) {
		$db = new Database();
		$phoneNumber = $array["countryCode"].$array["applicant_phone_number"];
	...
	function insert_new_web_form_applicant ($array,$user_id){
		$db = new Database();
		...
		//assign_new_file($array["applicant_cv_id_FK"]);
	...
	function update_applicant($array,$user_id){
		...
		`applicant_visa_type_FK` = '".$o_visa."',
		`applicant_current_employment_status` = '".$array["applicant_current_employment_status"]."',
		`applicant_current_employment_poistion` = '".$o_position."',
		...
		`applicant_profession_FK` = '".$o_profession."',
		...
		`applicant_board_certified_id_FK` = '".$o_board."',
...
WHERE active_status = 1 AND request_state_id_FK <> 5
...
INNER JOIN module_applicant ON module_applicant.applicant_id = emp_app_flow_has_applicant.applicant_id_FK WHERE flow_id_FK = '".$flow_id."'";
	...
	function get_flow_id($request_id){
		$db		=new Database();
		$query="SELECT * FROM `setup_emp_request_has_opening` WHERE request_id_FK = '".$request_id."'";
		$db->query($query);
	 	$result = $db->resultset();
	 	return $result;
	}

	function get_applicant_license($applicant_id)
	{
		$array_single=array();
		
		$db=new Database();
		
		$query="SELECT * FROM `applicant_has_license` 
		INNER JOIN `applicant_license` ON `applicant_license`.`license_id` = `applicant_has_license`.`license_id_FK` WHERE `applicant_id_FK`='".$applicant_id."'";
		...
		{
			array_push($array_single,$result[$i]["license_id_FK"]);
		}
		return $array_single;
	}

	function add_applicant_license($applicant_id,$license_id)
	{
		$query="INSERT INTO `applicant_has_license` 
		(
			`applicant_id_FK`,
			`license_id_FK`
		) 
	...
	function remove_all_applicant_license($applicant_id)
	{
		$db=new Database();
		$query="DELETE FROM `applicant_has_license` WHERE `applicant_id_FK`='".$applicant_id."'";
		$db->query($query);
		$db->execute();
	}
}
