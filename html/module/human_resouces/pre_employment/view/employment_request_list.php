...
<div class="page-header">
  <div class="row">
    <div class="col-md-4 text-xs-center text-md-left text-nowrap">
      <h1><i class="page-header-icon ion-ios-pulse-strong"></i>All Request Approval List</h1>
    </div>
  </div>
</div>
				...
				<table class="table table-responsive table-bordered table-primary" name="data_grid">
					...
					<tbody>
						...
						$request_inner_obj		=$requestObj->list_of_employment_requests_inner($request_obj[$i]["request_id"]);
							...
							?>
						<tr>
							<?php if ($request_obj[$i]["request_state_id_FK"] == 2) { ?>
								<td><a href="#" onclick="sourcing_modal('<?php echo $request_id; ?>','<?php echo $request_state_id; ?>')"><?php echo $request_obj[$i]["request_ref_number"]; ?><a></td>
							<?php } else if ($request_obj[$i]["request_state_id_FK"] == 4) { ?>
								<td><a href="#" onclick="matching_modal('<?php echo $request_id; ?>','<?php echo $request_state_id; ?>')"><?php echo $request_obj[$i]["request_ref_number"]; ?><a></td>
							...
							<?php } else { ?>
								<td><a href="#" onclick="open_modal('<?php echo $request_id; ?>','<?php echo $request_state_id; ?>')"><?php echo $request_obj[$i]["request_ref_number"]; ?><a></td>
							<?php } ?>
