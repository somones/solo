...
$customer_obj				=$customerObj->list_of_customer($_SESSION['employee_id']);
				...
				<table class="table table-responsive table-bordered table-primary" name="data_grid">
					...
					<tbody>
						<?php for($i=0;$i<count($customer_obj);$i++) { 
							...
							?>
							<tr>
								<td><i class="fa fa-edit" style="cursor:pointer" title="Edit Security Group" onclick="get_customer_form('<?php echo $customer_obj[$i]['customer_id']; ?>')"></i></td>
								<td><i class="fa fa-times" style="cursor:pointer" title="Edit Security Group" onclick="delete_customer('<?php echo $customer_obj[$i]["customer_id"]; ?>')"></i></td>							
